...
function getAccount(){
  return document.getElementById("account-select").value;
}
...
document.getElementById("open-mail").addEventListener("click",function(){
  ...
  var from=currentDraft.from;
  var subject=encodeURIComponent(currentDraft.subject);
  var body=encodeURIComponent(currentDraft.body);
  window.location.href="mailto:?from="+encodeURIComponent(from)+"&subject="+subject+"&body="+body;
});
...
function startMic(){
  ...
  function startRec(){
    if(!listening)return;
    rec=new SR();rec.lang=lang;rec.continuous=false;rec.interimResults=true;
    rec.onresult=function(e){
      var full="";
      for(var i=0;i<e.results.length;i++) full+=e.results[i][0].transcript+" ";
      var t=full.trim();savedText=t;box.textContent=t;
    };
    ...
    rec.onend=function(){if(listening){setTimeout(startRec,200);}};
...
function stopMic(){
  ...
  document.getElementById("status-label").textContent="Select account & tap to speak";
...
function makeDraft(text){
  ...
  if(name)sys+=" The sender is: "+name+" ("+account+").";
  ...
  .then(function(data){
    ...
    currentDraft={subject:d.subject,body:d.body,from:account};
    document.getElementById("from-val").textContent=account;
