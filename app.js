...
function loadSettings(){
  document.getElementById("inp-key").value=localStorage.getItem("dw_key")||"";
  document.getElementById("inp-name").value=localStorage.getItem("dw_name")||"";
...
function startMic(){
  var key=localStorage.getItem("dw_key");
  if(!key){showToast("Add API key in Settings");go("settings");return;}
  var SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR){showToast("Use Safari on iPhone");return;}
  ...
  rec.onresult=function(e){var interim="";for(var i=e.resultIndex;i<e.results.length;i++){if(e.results[i].isFinal){finalText+=e.results[i][0].transcript+" ";}else{interim+=e.results[i][0].transcript;}}box.textContent=finalText+interim;};
  rec.onerror=function(e){stopMic();showToast("Mic error: "+e.error);};
...
function stopMic(){
  listening=false;if(rec){rec.stop();rec=null;}
...
function makeDraft(text){
  var key=localStorage.getItem("dw_key");
  var name=localStorage.getItem("dw_name")||"";
  ...
  .catch(function(err){document.getElementById("tbox").innerHTML='<div class="err">'+err.message+'</div>';})
  .finally(function(){btn.classList.remove("busy");document.getElementById("mic-icon").textContent="🎙";document.getElementById("status-label").textContent="Tap to speak";});
}
